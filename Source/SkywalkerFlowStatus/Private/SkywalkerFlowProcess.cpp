...
void ASkywalkerFlowProcess::SwitchFlowProcess(TSubclassOf<ASkywalkerFlowProcess> NewFlowProcessClass)
{
	USkywalkerFlowStatusSubsystem *FlowStatusSubsystem = GetWorld()->GetGameInstance()->GetSubsystem<USkywalkerFlowStatusSubsystem>();

	if (FlowStatusSubsystem != nullptr)
	{
		FlowStatusSubsystem->SwitchFlowProcess(NewFlowProcessClass);
	}
}
